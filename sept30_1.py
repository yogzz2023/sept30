...
def main():
    ...
    # >>> Moved initialize_tracks inside the loop to handle dynamic track initialization
    ...
    for group_idx, group in enumerate(measurement_groups):
        ...
        # >>> Added: Initialize tracks for the current group
        # Only use the current group for initialization
        ...
        if len(group) > 1:  # Multiple measurements in the group
            ...
            for i, (rng, azm, ele, mt, md,*rest) in enumerate(group):
                ...
                for track_id, track in enumerate(tracks):
                    ...
                    # >>> Added: Handle 'free' state by setting it to 'unknown'
        ...
        # >>> Added: Update miss counts and remove tracks if necessary
        for track_id in list(miss_counts.keys()):
            if track_id not in firm_ids:
                ...
                if miss_counts[track_id] > 3:  # Adjust this threshold as needed
                    ...
                    print(f"hit_counts before deletion: {hit_counts}")
                    print(f"Trying to delete track_id: {track_id}")

                    if track_id in hit_counts:
                        del hit_counts[track_id]
                    else:
                        print(f"track_id {track_id} not found in hit_counts")
    ...
    # >>> Added: Check and manage track deletions based on miss counts
    ...
    for track in tracks:
        if track:
            ...
            if 'Sf' in track:
                ...
                print(f"'Sf' key not found in track. Current track contents: {track}")
